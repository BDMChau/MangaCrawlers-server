...
public interface GenreRepos extends JpaRepository<Genre, Long> {
    @Query("SELECT new serverapi.Query.DTOs.TablesDTOs.GenreDTO(g.genre_id, g.genre_name, g.genre_color, g.genre_description) FROM " +
    ...
    @Query("SELECT new serverapi.Query.DTOs.TablesDTOs.MangaGenreDTO(g.genre_id, g.genre_name, g.genre_color, g.genre_description, m.manga_id) FROM " +
            "Manga m JOIN m.mangaGenres mg ON mg.manga = m.manga_id JOIN Genre g ON g.genre_id = mg.genre  ")
