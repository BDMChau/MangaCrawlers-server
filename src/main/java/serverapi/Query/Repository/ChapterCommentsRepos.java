...
public interface ChapterCommentsRepos extends JpaRepository<ChapterComments,Long> {
    ...
    @Query("SELECT new serverapi.Query.DTO.ChapterCommentsDTO(u.user_id, u.user_name, u.user_email, u.user_avatar, " +
            ...
            " FROM ChapterComments cm JOIN cm.chapter c JOIN cm.user u JOIN Manga m ON m.manga_id = c.manga WHERE c.chapter_id =:chapter_id) LIMIT:from, :to")
    List<ChapterCommentsDTO> getCommentsChapter(@Param ("chapter_id") Long chapter_id, @Param("from")int from, @Param("to")int to);
