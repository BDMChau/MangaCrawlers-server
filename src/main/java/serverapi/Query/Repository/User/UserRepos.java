...
public interface UserRepos extends JpaRepository<User, Long> {
    ...
    @Query("SELECT new serverapi.Query.DTOs.TablesDTOs.ChapterDTO(c.chapter_id, c.chapter_name, c.created_at) FROM Chapter c JOIN c.readingHistories rd WHERE c.chapter_id =?1 ")
    List<ChapterDTO> findChapterByReadingHistory(Long chapter_id);

    @Query("SELECT new serverapi.Query.DTOs.TablesDTOs.MangaDTO(m.manga_id, m.manga_name, m.thumbnail) FROM Manga m JOIN m.readingHistories rd WHERE m.manga_id =?1 ")
    List<MangaDTO> findMangaByReadingHistory(Long manga_id);

    @Query("SELECT new serverapi.Query.DTOs.FeaturesDTOs.ReportDTOs.ReportUserFollowMangaDTO(COUNT(u.user_id), a.author_id, a.author_name, m.manga_id, m.manga_name, m.thumbnail, m.stars, m.views, m.date_publications)"+
...
//    List<UserTransgroupDTO> getUsersTransgroup(Long transgroup_id);
    ...
    @Query("SELECT new serverapi.Query.DTOs.TablesDTOs.UserTransGroupDTO(u.user_id, u.user_name, u.user_email, u.user_avatar, tg.transgroup_id)" +
...
//    Optional<UserDTO> getUser(Long UserDTO);
    ...
    @Query("SELECT new serverapi.Query.DTOs.FeaturesDTOs.ReportDTOs.ReportTopMangaDTO(a.author_id,a.author_name, m.manga_id, m.manga_name, m.thumbnail, m.stars, m.views, m.date_publications, m.created_at)" +
            "FROM Manga m JOIN m.author a ORDER BY m.views DESC ")
    List<ReportTopMangaDTO> findTopManga(Pageable pageable);


    @Query("SELECT new serverapi.Query.DTOs.FeaturesDTOs.ReportDTOs.UserRDTO(u.user_id, u.created_at) FROM User u")
